...
class App extends React.Component {
    ...
    return Object.keys(ChartPeriod).map((key) => {
      ...
      return (
        <button
          ...
          className={
            "btn btn-default " +
            (this.isActiveChartPeriod(chartPeriod) ? "active" : "")
          }
  ...
  renderWindowButtons() {
    return (
      <div className="window-buttons">
        <button type="button" className="close" onClick={() => this.closeApp()}>
          &times;
        </button>
      </div>
    );
  ...
  render() {
    if (!this.initialized) {
      return <div className="panel panel-default"></div>;
    }
    ...
    return (
      <div className="panel panel-default">
        <div
          className={"panel-heading " + (current !== null ? "selected" : "")}
          style={headerStyle}
        >
          <div className="align-center">
            <img className="h-16" src={financeLogo} alt="app logo" />
            &nbsp;&nbsp;{this.state.title}
          </div>
          <div className="align-center">
          ...
          <div className="align-center">
            ...
            <div className="window-buttons">{this.renderWindowButtons()}</div>
          </div>
        </div>
